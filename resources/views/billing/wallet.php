...
<div class="mb-6">
    ...
    <p class="mt-1 text-sm text-slate-500">Your company’s wallet, plan, seats and features. Candidates are always free.</p>
</div>
...
<div class="grid gap-6 lg:grid-cols-3">
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2">
        ...
        <?php if ($plan === null): ?>
            <p class="mt-3 text-sm text-slate-500">No plan yet. The Owner is free; compose a plan below to add staff seats and premium features.</p>
        <?php else: ?>
            ...
            <?php if ($billableSeats > $coveredSeats): ?>
                <div class="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700">You have <?= e($billableSeats) ?> staff but only <?= e($coveredSeats) ?> seat(s) funded. Add seats or re-compose your plan.</div>
            <?php endif; ?>
        <?php endif; ?>
    </div>
</div>
...
<div class="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    <h2 class="text-lg font-semibold text-slate-900"><?= $plan === null ? 'Compose your plan' : 'Re-compose / renew' ?></h2>
    <p class="mt-1 text-sm text-slate-500">Pick billable seats (every staff member beyond the free Owner) and the premium features you need. Basics (Jobs, Interviews) are always included.</p>
    ...
    <form method="post" action="/billing/compose" class="mt-4" onsubmit="return confirm('Activate this plan now? The cost is debited from your wallet and the term runs one month.');">
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            <div>
                <label class="block text-sm font-medium text-slate-700">Billable seats</label>
                ...
                <p class="mt-1 text-xs text-slate-400"><?= e($money($seatPriceCents)) ?> / seat / month. Minimum = current staff (<?= e($billableSeats) ?>).</p>
            </div>
            <div>
                <span class="block text-sm font-medium text-slate-700">Premium features</span>
        ...
        <!-- Pre-activation review (docs/WALLET_AND_BILLING.md §4) -->
        <div class="mt-5 rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-3 text-sm text-indigo-800">
            <strong>Before you activate:</strong> review whether you need any extra features now. Add-ons added later still <em>expire with this same plan</em> — you won’t get a fresh month for them.
        </div>
...
<!-- Add-ons (only features not already active) -->
<?php if ($active): ?>
    ...
    <div class="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p class="mt-1 text-sm text-slate-500">Enable an extra feature now. It’s charged immediately and <strong>expires with your current plan</strong> on <?= e($plan['period_end'] ?? '') ?> UTC.</p>
        <?php if ($available === []): ?>
            <p class="mt-3 text-sm text-slate-400">All premium features are already active in your plan.</p>
        <?php else: ?>
