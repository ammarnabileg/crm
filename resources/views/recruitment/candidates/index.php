...
/** @var bool $searching */
/** @var array{q:string,min_score:int,recommendation:string,skill:string,skill_min:int} $filters */
/** @var array<string,array{label:string,weight:int}> $skills */
/** @var bool $aiSearch */
$aiResults = ($filters['min_score'] ?? 0) > 0 || ($filters['recommendation'] ?? '') !== '' || ($filters['skill'] ?? '') !== '';
?>
<div class="mb-6">
...
<!-- ATS keyword search (CV-based, no AI required) -->
<form method="get" action="/candidates" class="mb-4 flex flex-wrap items-end gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
    <div class="grow">
        <label class="mb-1 block text-xs font-medium text-slate-500">Search CV / candidate</label>
        <input name="q" value="<?= e($filters['q']) ?>" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" placeholder="Name, email, skill or keyword from the CV…">
    </div>
    <button class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700">Search</button>
    <?php if ($filters['q'] !== ''): ?><a href="/candidates" class="text-sm text-slate-500 hover:underline">Clear</a><?php endif; ?>
</form>

<!-- Advanced search by AI assessment (needs AI / OpenAI key) -->
<form method="get" action="/candidates" class="mb-4 flex flex-wrap items-end gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
    <div class="w-full text-xs font-semibold uppercase tracking-wide text-slate-400">AI assessment search<?= $aiSearch ? '' : ' · inactive (add an OpenAI key)' ?></div>
    <div>
        <label class="mb-1 block text-xs font-medium text-slate-500">Min score</label>
        <input type="number" name="min_score" min="0" max="100" value="<?= e($filters['min_score'] ?: '') ?>" <?= $aiSearch ? '' : 'disabled' ?> class="w-24 rounded-lg border border-slate-300 px-3 py-2 text-sm disabled:bg-slate-50" placeholder="0-100">
    </div>
    <div>
        <label class="mb-1 block text-xs font-medium text-slate-500">Recommendation</label>
        <select name="recommendation" <?= $aiSearch ? '' : 'disabled' ?> class="rounded-lg border border-slate-300 px-3 py-2 text-sm disabled:bg-slate-50">
    ...
    <div>
        ...
        <select name="skill" <?= $aiSearch ? '' : 'disabled' ?> class="rounded-lg border border-slate-300 px-3 py-2 text-sm disabled:bg-slate-50">
            <option value="">Any</option>
    ...
    <div>
        <label class="mb-1 block text-xs font-medium text-slate-500">Skill ≥</label>
        <input type="number" name="skill_min" min="0" max="100" value="<?= e($filters['skill_min'] ?: '') ?>" <?= $aiSearch ? '' : 'disabled' ?> class="w-20 rounded-lg border border-slate-300 px-3 py-2 text-sm disabled:bg-slate-50" placeholder="0">
    </div>
    <?php if ($aiSearch): ?>
        <button class="rounded-lg bg-slate-800 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700">AI search</button>
    <?php endif; ?>
    <?php if ($aiResults): ?><a href="/candidates" class="text-sm text-slate-500 hover:underline">Clear</a><?php endif; ?>
</form>
        ...
        <table class="w-full text-sm">
            <thead class="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-400">
                <tr>
                    ...
                    <th class="px-5 py-3"><?= $aiResults ? 'AI score' : 'Applications' ?></th>
                </tr>
            </thead>
                ...
                <?php foreach ($candidates as $c): ?>
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-5 py-3 text-slate-600">
                            <?php if ($aiResults): ?>
                                <span class="font-semibold text-slate-800"><?= e($c['fit_score'] ?? 0) ?>/100</span>
