...
/**
 * The Infrastructure-layer, real, seedable in-memory {@see DepartmentResolver} — the safe default the
 * {@see \Nizam\Runtime\Infrastructure\RuntimeServiceProvider} binds until the production org-structure
 * adapter exists.
 *
 ...
 * as an unroutable production intent would. Routing is tenant-scoped and honours a department hint: a
 * (tenant, intent, hint) route takes precedence over the plain (tenant, intent) route, so a caller can
 * bias routing without changing the intent. It is a working adapter, not a stub.
 */
