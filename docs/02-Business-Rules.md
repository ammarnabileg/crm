...
flowchart TD
    ...
    D --> E[(MySQL: FK, UNIQUE, NOT NULL constraints)]
...
|-------|----------------|---------------------|
| Database | `database/migrations/*` | Uniqueness, referential integrity, config-driven status/type FKs (no ENUM), NOT NULL |
...
| Policy gates | `app/Services/Rbac/AccessControl.php` | Context-aware ("own record", "in my workspace") rules |
| Validator | `app/Core/Validator.php` | Input shape, required/length/format, uniqueness pre-check |
| Service | `app/Services/*` | Multi-step transactional invariants (e.g. workspace provisioning) |
...
sequenceDiagram
    ...
    P-->>C: allow / deny
    C->>M: persist
    M->>M: inject workspace_id (BR-041)
    M->>DB: prepared statement
    DB-->>M: enforce UNIQUE/FK (BR-002, BR-042…)
```
...
- **BR-005** A user has one of three statuses via `users.user_status_id` (FK → `lookup_values`, category `user_status`, config-driven — no hard-coded ENUM): `active`, `suspended`, `pending`. Only `active` users may authenticate. `suspended` users are denied login with a generic message; `pending` users must complete verification first.
...
- **BR-040** Every tenant-scoped query is automatically constrained by the active workspace id (`workspace_id`). A tenant-scoped model query with **no active tenant fails closed** (throws) — it never silently returns all rows.
- **BR-041** On insert, tenant-scoped models inject the active `workspace_id` automatically; application code must not set it to a different workspace.
...
- **BR-043** The `workspaces` table is the tenant root and is global; it cannot scope to itself. All other tenant tables carry a `workspace_id` foreign key with an index.
- **BR-044** Uniqueness of tenant data is per-workspace, not global, wherever a value is naturally tenant-local — e.g. `workspaces` slug is global, but role slug is unique per workspace `(workspace_id, slug)`, settings key is `(workspace_id, key)`, and a candidate may apply once per job `(workspace_id, job_id, user_id)`.
- **BR-045** A user may belong to multiple workspaces via multiple `memberships` rows; exactly one workspace is "active" per request, resolved from the session `active_workspace_id` and validated against an `active` membership.
- **BR-046** Switching the active workspace requires the user to hold an `active` membership in the target workspace; otherwise the switch is denied.
- **BR-047** Deleting a workspace cascades to its tenant-owned rows (memberships, tenant roles, subscriptions, AI credentials, settings, and all recruitment data) via `ON DELETE CASCADE`; a workspace cannot be deleted while it is referenced as an `owner` constraint requires (RESTRICT on `workspaces.owner_id`).
...
- **BR-061** A user's **effective permissions** in the active tenant are the union of: (a) permissions of their global roles in `user_roles`, and (b) permissions of the tenant roles attached to their active `membership`; each role is expanded up its `parent_id` inheritance chain.
...
- **BR-063** `roles.workspace_id = NULL` denotes a **global** role (assigned via `user_roles`); a non-null `workspace_id` denotes a **tenant** role (assigned via `membership_roles`). The two assignment paths never mix.
...
- **BR-050** Any authenticated user may create a company. Creation is **atomic**: it inserts the `workspaces` row, the owner `membership`, the default tenant roles, assigns the Owner role, and starts a trial subscription — all in one transaction. If any step fails, none persist.
- **BR-051** The creator of a company becomes its Owner and is recorded in `workspaces.owner_id`. A workspace always has exactly one owner reference.
- **BR-052** A new workspace starts in status `trial`; the status is config-driven via `workspaces.workspace_status_id` (FK → `workspace_statuses`, no hard-coded ENUM), with seeded values `trial`, `active`, `suspended`, `canceled`.
- **BR-053** A member's status is config-driven via `memberships.membership_status_id` (FK → `lookup_values`, category `membership_status`, no hard-coded ENUM): `active`, `invited`, or `suspended`. Only `active` members can use the tenant; `invited` members must accept first.
- **BR-054** A user cannot have two memberships in the same workspace — `(workspace_id, user_id)` is unique.
...
- **BR-100** A job (`jobs`) belongs to exactly one workspace and is tenant-scoped. Its `status` is config-driven via a `*_status_id` FK (no hard-coded ENUM): `draft`, `open`, `paused`, `closed`, `archived`.
- **BR-101** Only a job in `open` status accepts new applications. Applying to a `draft`, `paused`, `closed`, or `archived` job is rejected.
- **BR-102** A job's slug is unique per workspace `(workspace_id, slug)`, enabling stable public posting URLs without colliding across tenants.
...
- **BR-120** An application (`applications`) links one candidate `user` to one `job` within a workspace. A candidate may apply to a given job at most once — `(workspace_id, job_id, user_id)` is unique.
...
- **BR-140** An interview (`interviews`) is tied to one application and one job within a workspace; its `type` and `mode` are config-driven via `*_id` FKs (no hard-coded ENUM): type `ai`, `human`, or `panel`; mode `video`, `phone`, `onsite`, or `ai_async`.
...
- **BR-160** The platform stores **no** AI provider keys of its own. Each workspace stores its own credentials in `tenant_ai_keys`, encrypted at rest with AES-256-GCM.
...
- **BR-162** A tenant has at most one credential row per provider — `(workspace_id, provider)` is unique — and may mark one as `is_default`.
...
- **BR-181** Payments (`payments`) reference a `gateway` and a `gateway_reference`; payment `status` is `pending`, `succeeded`, `failed`, or `refunded`. `(workspace_id, gateway_reference)` is indexed for reconciliation.
...
- **BR-200** Security- and business-significant events are written to `activity_logs` with the actor (`user_id`), `action`, `subject_type`/`subject_id`, `ip`, and `user_agent`. Audit rows are append-only.
...
| BR-044, BR-054 | `memberships` | `UNIQUE(workspace_id, user_id)` |
| BR-044, BR-063 | `roles` | `UNIQUE(workspace_id, slug)`, `workspace_id` nullable FK→`workspaces` |
| BR-061, BR-066 | `role_permissions`, `membership_roles`, `user_roles` | composite PKs, FK→`roles`/`permissions` |
| BR-072 | `subscriptions` | `amount`, `currency` columns (snapshot), `plan_id` FK→`plans` RESTRICT |
| BR-102, BR-103 | `jobs` | `UNIQUE(workspace_id, slug)`, config-driven `*_status_id` FK (no ENUM), `published_at`/`closed_at` |
| BR-120 | `applications` | `UNIQUE(workspace_id, job_id, user_id)` |
| BR-126 | `application_events` | FK→`applications` CASCADE, append-only |
| BR-162 | `tenant_ai_keys` | `UNIQUE(workspace_id, provider)`, `credentials` encrypted TEXT |
...
- Workspace/members: `workspace.view/update`, `members.view/invite/update/remove`.
- Roles: `roles.view/manage` (BR-064, BR-065).
- Billing: `billing.view/manage` (BR-070, BR-180).
- AI: `ai.view/manage` (BR-160, BR-162).
- System (platform-ops, super-admin only): `system.manage` — gates `/system/*` (diagnostics, maintenance mode, backups/restore, environment editor, log viewer).
...
- Platform (planned): `platform.workspaces.*`, `platform.users.*`, `platform.plans.manage`, `platform.diagnostics` (BR-062).
...
- Role slug: `required|max:120|unique:roles,slug` scoped per workspace (BR-044).
...
- **No active tenant on a tenant route**: middleware redirects to workspace selection; a tenant-scoped model accessed with no tenant throws (BR-040) — fail closed, never leak.
- **Super-admin with no active workspace**: allowed to operate platform-wide; tenant-scoped reads must use `withoutTenantScope()` explicitly (BR-042, BR-062).
- **Concurrent workspace creation** with a colliding slug: the unique index rejects the loser; the service retries with a suffixed slug inside the transaction (BR-050).
- **Race on duplicate application**: two simultaneous applies hit `UNIQUE(workspace_id, job_id, user_id)`; one succeeds, the other is caught and reported as "already applied" (BR-120).
...
- Tenant scoping relies on the `workspace_id` index present on every tenant table, so the mandatory filter is index-backed (BR-040).
