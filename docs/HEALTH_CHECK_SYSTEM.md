...
- The overall status is the **worst** contributing result; `info` results and
  `skip` results never raise it.
- The Health Report carries: overall status, a counts summary
  (pass/warn/fail/skip), the full ordered list of probe results, the total run
  duration, and a generation timestamp.
- The report is **machine-readable first** (so deploy tooling and Phase 15 can
  consume it) and rendered for humans second.
- **Fail-closed for install:** the Installer treats *anything other than*
  `HEALTHY`/`DEGRADED-with-acknowledgement` as a stop (`Installer.md` §9). For
  production readiness, the deploy health check (`DEPLOYMENT_GUIDE.md` §10–§11)
  treats `UNHEALTHY` as a blocker.
...
Probes register through the same explicit mechanism as the rest of the kernel —
no auto-scan, no reflection sweep over the filesystem (`ARCHITECTURE.md` §6).

- **Kernel-provided probes** (runtime, storage, etc.) are registered by the Core
  Kernel's health service provider at boot.
- **Module-contributed probes** are registered by the owning module's **service
  provider** during the register/boot phase (`ARCHITECTURE.md` §7;
  `Core_Kernel.md` §2). The Mail module contributes the mail probe; the AI Engine
  contributes the AI-providers probe; the Integration Platform contributes the
  API-providers and SSL probes; the Database module contributes the MySQL probes.
- Registration is **additive**: a new probe is added by registering it in a
  provider — touching **no** existing probe and **no** aggregator code
  (`ARCHITECTURE.md` §9). Probe `key`s **MUST** be unique; a duplicate key is a
  defect.
- A probe **MAY** declare itself **conditional** (e.g. the Queue probe `skip`s
  when no queue driver is configured) so optional capabilities degrade gracefully
  rather than reporting a false failure (`ARCHITECTURE.md` §5).

> **EXAMPLE — illustrative registration in a module provider:**
> ```php
> // In a module's ServiceProvider::boot()
> $registry->register(new MailTransportProbe($mailConfig));
> ```
...
database tables; persistence of snapshots over time is a Phase 15 concern
...
4. **Deploy / readiness check.** A release MUST satisfy a **green health check**
   before cutover (`DEPLOYMENT_GUIDE.md` §10, §11). The machine-readable report is
...
(`PERMISSION_MODEL.md` §6). Health endpoints **MUST NOT** be exposed
unauthenticated; the only pre-auth use is *inside* the install flow, which is
itself gated by install-state and the lock (`Installer.md` §6).

---

## 7. Execution Model

- **On-demand, synchronous** for the Installer and the "Run Full Diagnostics"
  button: run all probes, aggregate, return one report.
- **Selective runs** are supported: a caller MAY run a single probe or a group
  (by `key` or `group`) — e.g. the Database module re-checking only `db.*` after a
  connection change.
- **Bounded:** each probe is time-boxed (§1); the total run is the sum of bounded
  probe times. A probe exceeding its timeout returns `warn`/`fail` (per its
  severity) with a timeout reason, never a hang.
- **Isolated:** probes run independently; an exception in one is caught, recorded
  as a `fail` for that probe, and the run continues (§1, principle 4).
- **Cache-aware (Phase 15):** continuous monitoring MAY cache recent results to
  avoid hammering dependencies; the Installer and explicit diagnostics always run
  **fresh**. Caching policy is owned by Observability, not the kernel.
...
- A probe **MUST** be **read-only**: no writes, no outbound sends (alert delivery
  is the Integration Platform's job via Phase 15 — `Observability.md` §2), no
  tenant data access.
- Health/diagnostics endpoints **MUST** be permission-gated
  (`system.health.view` / `system.diagnostics.run`) and Platform-Context-only;
  there is **no** unauthenticated health endpoint except inside the gated install
  flow (`SECURITY_GUIDE.md` §3; `Installer.md` §6).
- A probe failure or exception **MUST NOT** crash the host request/page; it is
  contained as a single failed result (§1, §7).
...
**consumer and aggregator** (`Observability.md` §1):

- The **Health Center** reads the probe surface to build the real-time, per-module
  health view and to snapshot results as time-series (`HEALTH_CENTER.md`,
  `OBSERVABILITY.md`; *Health Snapshot* — `Observability.md` §8).
- The **Alert Engine** turns degraded/unhealthy results into alerts, delivered
  **only** through the Integration Platform — the kernel never sends anything
  itself (`Observability.md` §2, §7).
- Probe failures correlate with the global Error Handler's `request_id` for
  **Error Tracking** (`ERROR_TRACKING.md`, `ERROR_HANDLING_GUIDE.md`).
- Until Phase 15 ships, the Health Checker still fully serves the **Installer**
  and the **deploy readiness** gate; the operations console is additive on top.
