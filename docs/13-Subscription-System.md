...
| `CompanyService::startTrialSubscription()` | `app/Services/Tenancy/CompanyService.php` | Bootstraps the trial subscription on the first active plan at company creation. |
| `DatabaseSeeder` | `database/seeders/DatabaseSeeder.php` | Idempotently seeds the **Standard** plan (50.00 SAR / monthly / 14-day trial). |
| `Company::activeSubscription()` | `app/Models/Company.php` | Resolves the current `trialing|active` subscription for a company. |
| Billing layer | see [14](14-Billing-System.md) | Generates invoices and applies payments that drive state transitions. |

**How it fits.** A subscription is **tenant-scoped** (`Subscription::$tenantScoped = true`), so reading "my subscription" is automatically isolated by `company_id`; platform-wide queries (a Super Admin reviewing all subscriptions, or `Company::activeSubscription()` for an arbitrary company) use `withoutTenantScope()`. Plans are **global** (`Plan::$tenantScoped = false`) because the same catalogue is shared by every tenant. Crucially, `amount` and `currency` are **snapshotted onto the subscription** at subscribe time, so later edits to a plan's price never retroactively change an existing subscription.
...
The five states are exactly the values of the `subscriptions.status` ENUM: `trialing`, `active`, `past_due`, `canceled`, `expired`. `Subscription::isActive()` treats `trialing` and `active` as "in good standing"; `onTrial()` is true only for `trialing`.
...
At company creation, `startTrialSubscription()` selects the first plan with `is_active = 1` ordered by `sort_order`, then inserts a subscription with `status='trialing'` and `trial_ends_at = now + plan.trial_days` when `trial_days > 0`, or `status='active'` immediately when `trial_days = 0`. `amount` and `currency` are copied from the plan.
...
2. **Ships with one plan: "Standard"**, slug `standard`, **50.00 SAR**, `interval='monthly'`, `trial_days=14`, `features={ai_providers,members,roles,activity_log: true}`, `limits={max_members:25}`. Seeded idempotently (skips if `slug='standard'` exists).
3. **Every company has at most one effective subscription in good standing.** `Company::activeSubscription()` returns the latest `trialing|active` row.
...
- **`plans`** (GLOBAL): `id`, `name`, `slug VARCHAR(120) UNIQUE`, `description`, `price DECIMAL(10,2)`, `currency VARCHAR(3) DEFAULT 'SAR'`, `interval ENUM('monthly','yearly')`, `trial_days INT`, `features JSON`, `limits JSON`, `is_active TINYINT(1)`, `is_public TINYINT(1)`, `sort_order INT`, timestamps. Index `plans_is_active_index`.
- **`subscriptions`** (tenant): `id`, `company_id → companies(id) ON DELETE CASCADE`, `plan_id → plans(id) ON DELETE RESTRICT`, `status ENUM('trialing','active','past_due','canceled','expired')`, `amount DECIMAL(10,2)`, `currency`, `trial_ends_at`, `starts_at`, `ends_at`, `canceled_at`, timestamps. Indexes on `company_id`, `plan_id`, `status`.
...
- **Downgrade guard**: target `plan.limit('max_members')` must be `>= Company::membersCount()`; otherwise rejected with a clear message to remove members first.
- **Cancellation**: confirmation required; sets `canceled_at=now`, computes `ends_at` from the current cycle.
- **Plan authoring** (platform): `slug` unique, `price >= 0`, `currency` ISO-4217 (3 chars), `interval in:monthly,yearly`, `trial_days >= 0`, `features`/`limits` valid JSON objects.
...
- **CSRF** on every mutating route; **audit** every plan change/cancellation to `activity_log` with actor and before/after plan.
- **Integrity**: `plan_id` `RESTRICT` and `company_id` `CASCADE` keep referential integrity; snapshotting prevents price-tampering disputes.

## Performance

- **Hot path — "is this tenant in good standing?"**: a single indexed lookup on `subscriptions(company_id, status)`; cache the active subscription per request after the first resolve.
...
- All FKs (`company_id`, `plan_id`) and `status` are indexed for fast renewal/dunning sweeps.
...
- **Annual plans & promotions**: already supported by `interval='yearly'` and additional plan rows; add coupon/discount tables without touching the lifecycle.
