...
- `App\Services\Rbac\RbacManager` (`app/Services/Rbac/RbacManager.php`) — the **provisioner** that materialises permissions and roles from `config/rbac.php` into the database (at install time and whenever a workspace is created).
...
- Let a workspace build a custom role in a UI without deploying code.
...
The policy-gate layer exists because flat permissions cannot answer context-aware questions like "may this user edit *their own* profile" or "may this recruiter move *an application that belongs to their workspace*". Those are the "dynamic permissions" — closures that inspect the subject, not just a flag.
...
| `RbacManager` | `app/Services/Rbac/RbacManager.php` | Provisioning: `syncPermissions()`, `ensureSuperAdminRole()`, `provisionWorkspaceRoles()`, `assignGlobalRole()`, `assignMembershipRole()`, `syncRolePermissions()`. Model-free, takes an explicit `Database` so the *same* code runs in the installer and at runtime. |
...
Roles live in the `roles` table and are distinguished **solely by `workspace_id`**:

- **Global role** — `workspace_id IS NULL`. Assigned to a user directly via the `user_roles` pivot. The canonical example is `super-admin`. Global roles apply platform-wide regardless of which tenant is active.
- **Tenant role** — `workspace_id = <id>`. Assigned through a user's `membership` via the `membership_roles` pivot. Examples: `owner`, `admin`, `member` (and the planned `hr-manager`, `recruiter`, `hiring-manager`, `interviewer`, `candidate`). Tenant roles only apply inside their own workspace.

This is why the `workspaces` table is global and cannot scope to itself, and why the `roles` table carries a nullable `workspace_id` with a unique `(workspace_id, slug)` constraint: every workspace gets its own `owner`, its own `admin`, etc., as independent rows.
...
| Workspace | `workspace.view`, `workspace.update` |
...
`config/rbac.php` ships three system tenant-role templates today — `owner` (permissions `'*'`), `admin` (everything except `billing.manage` and ownership), `member` (read-mostly) — plus the global `super-admin`. The remaining personas (`hr-manager`, `recruiter`, `hiring-manager`, `interviewer`, `candidate`) are defined as data in the same file as their domain permissions land. Because the templates are data, a deployment can add or re-tune a default role by editing config and re-seeding, and a workspace can clone/extend a role at runtime through the role editor gated by `roles.manage`.

## Workflow

### Provisioning (write path)

1. **Install** (`InstallManager` → `RbacManager`): `syncPermissions()` upserts the catalogue into `permissions`; `ensureSuperAdminRole()` creates the `super-admin` global role (`workspace_id = NULL`, `is_system = 1`) and grants it *every* permission id.
2. **Workspace creation** (`WorkspaceService::create()` → `RbacManager::provisionWorkspaceRoles($workspaceId)`): inside the same DB transaction that creates the workspace and the owner membership, every template in `config/rbac.php`'s `tenant_roles` becomes a `roles` row for that workspace, its permissions are written to `role_permissions`, and `parent` links are resolved into `parent_id`. The owner membership is then granted the `owner` role via `membership_roles`.
3. **Assignment**: `assignGlobalRole($userId, $roleId)` writes `user_roles`; `assignMembershipRole($membershipId, $roleId)` writes `membership_roles`. Both are idempotent (guarded by an existence check).
...
flowchart TD
    ...
    F --> G{Cached for<br/>userId:workspaceId?}
    G -- Yes --> K["lookup key in cache map"]
    G -- No --> H["resolveRoleIds(user, workspaceId)"]
    H --> H1["global roles via user_roles"]
    H --> H2["tenant roles via active<br/>membership + membership_roles"]
    H1 --> I["expandWithAncestors()<br/>walk parent_id chains"]
    H2 --> I
    I --> J["SELECT permissions.key<br/>JOIN role_permissions<br/>WHERE role_id IN (...)"]
...
1. Cache key = `"{userId}:{workspaceId}"` (workspaceId `0` when no active tenant). Return early on a hit.
2. `resolveRoleIds($user, $workspaceId)`:
   - **(a)** global role ids from `user_roles WHERE user_id = ?`;
   - **(b)** if a tenant is active, the `memberships.id` for `(user, workspace)` with `status = 'active'`, then the role ids from `membership_roles WHERE membership_id = ?`;
...
4. A single `SELECT DISTINCT permissions.key … JOIN role_permissions … WHERE role_permissions.role_id IN (:ids)` flattens those roles into a permission map `[key => true]`, which is cached and returned.
...
2. **Global vs tenant is decided by `roles.workspace_id`.** `NULL` ⇒ global (assigned via `user_roles`); a value ⇒ tenant (assigned via `membership_roles`). Uniqueness is `(workspace_id, slug)`.
...
4. **Only the *active* membership contributes.** Tenant role resolution uses the currently active `workspace_id` (from `TenantManager`) and requires `memberships.status = 'active'`. Roles from other workspaces a user belongs to never leak into the current tenant's decision.
...
6. **Policy gates beat flat permissions.** If an ability has a registered gate, the gate decides — enabling context-aware ("own record", "in my workspace") rules that a boolean flag cannot represent.
7. **`is_system` roles are protected.** Roles seeded with `is_system = 1` (`super-admin`, `owner`, `admin`, `member`) must not be deletable or renamable by the role editor; the editor (gated by `roles.manage`) may only edit non-system roles or clone system ones. This guarantees a workspace can never delete its `owner` role and lock itself out.
...
|-------|-------------|-------|
| `roles` | `id`, `workspace_id` (FK→`workspaces` CASCADE, **NULL = global**), `parent_id` (FK→`roles` SET NULL), `slug`, `is_system`, `priority` | `UNIQUE(workspace_id, slug)`, `INDEX(parent_id)`. Single-parent inheritance via `parent_id`. |
| `permissions` | `id`, `key` (UNIQUE), `module_id` (FK→`system_modules`), `action`, `name` | `INDEX(module_id)`. The global catalogue; referenced by `key` in code (the module is the permission group). |
| `role_permissions` | `role_id` (FK→`roles` CASCADE), `permission_id` (FK→`permissions` CASCADE) | `PK(role_id, permission_id)`. What each role grants. |
| `membership_roles` | `membership_id` (FK→`memberships` CASCADE), `role_id` (FK→`roles` CASCADE) | `PK(membership_id, role_id)`. Tenant-role assignment. |
| `user_roles` | `user_id` (FK→`users` CASCADE), `role_id` (FK→`roles` CASCADE) | `PK(user_id, role_id)`. Global-role assignment. |
| `memberships` | `id`, `workspace_id`, `user_id`, `status` | `UNIQUE(workspace_id, user_id)`; only `status = 'active'` contributes roles. |

Cascade behaviour matters for correctness: deleting a workspace cascades its tenant `roles`, which cascades the relevant `role_permissions`/`membership_roles` rows; deleting a user cascades their `user_roles`/`membership` rows. No orphaned grants survive.
...
- `slug` — required, lowercase-kebab, unique within the workspace (`UNIQUE(workspace_id, slug)`); auto-derived from name if omitted.
- `description` — optional, max 255.
- `parent_id` — optional; must reference an existing role **in the same workspace** (or be null); must not create a cycle (the chosen parent may not be a descendant of the role being edited).
- `permissions[]` — each value must be a key that exists in the `permissions` catalogue; unknown keys are rejected (mirrors `RbacManager::provisionWorkspaceRoles()`, which filters unknown keys via `permissionKeyToId()`).
- `is_system` — never settable through the UI; only the provisioner sets it.
- `priority` — integer, default 0.

`RbacManager::syncRolePermissions()` is the write primitive: it deletes existing `role_permissions` rows for the role and re-inserts the de-duplicated set, so saving a role is an idempotent replace.
...
- **No active tenant** (fresh super admin, or a user who hasn't selected a workspace) → `workspaceId` is `null`; only global roles contribute. Cache key uses `'0'` for the workspace segment so platform-context permissions cache separately from any tenant.
- **Suspended/invited membership** → excluded by `status = 'active'` in `resolveRoleIds()`; an invited-but-not-joined user gets none of that workspace's tenant roles.
...
- **Super-admin bypass is narrow and explicit.** It depends solely on holding a *global* `super-admin` role (`user_roles` → `roles.slug = 'super-admin'` with `workspace_id IS NULL`), checked by a single query in `User::isSuperAdmin()`. There is no environment flag or hidden backdoor.
...
- **Tenant correctness is a security property.** Resolution is bound to the active `workspace_id`; combined with model-layer scoping ([08 — Multi-Tenant](08-Multi-Tenant.md)) this prevents a user from exercising one workspace's roles against another's data.
- **Audit.** Role assignment/changes and sensitive RBAC events should be written to `activity_logs` (actor, subject, ip) per §13.

## Performance

- **Per-request cache.** `AccessControl::$cache` keys the fully-resolved `[key => true]` map by `"userId:workspaceId"`. The first `can()` in a request pays for resolution; every subsequent check (controllers, repeated view gates) is an array lookup. A page that renders dozens of `can()`-gated elements performs **one** resolution, not dozens.
- **Bounded queries.** Resolution is: one `user_roles` read, one `memberships` lookup, one `membership_roles` read, an ancestor walk (one tiny `parent_id` read per distinct role — chains are short, typically depth 1–2), and one final flatten join. All hit indexed columns (`user_roles` PK, `memberships UNIQUE(workspace_id,user_id)`, `membership_roles` PK, `role_permissions` PK, `roles.parent_id` index).
...
- Super admin → `allows()` true for any ability, including undefined ones, without touching `role_permissions`.
...
- `provisionWorkspaceRoles()` creates the configured roles, wires `parent_id`, and resolves `'*'` to all permissions.
...
- Switching active workspace changes the effective permission set (roles from workspace A do not apply in workspace B).
- Attempt to delete an `is_system` role → rejected.
- A user with an `invited` (non-active) membership receives none of that workspace's permissions.
...
- **Custom roles per workspace** via the `roles.manage` editor (clone a system role, tweak permissions) — already supported by the data model and `syncRolePermissions()`.
