...
| R-14 | **Observability blind spots** — failures in async/event flows or cross-context sagas are hard to trace and diagnose. | operational | Medium | Medium | **Medium** | OpenTelemetry traces/metrics/logs end-to-end; correlation IDs propagated through events and sagas; structured JSON logging (Monolog / PSR-3); dashboards and SLO/alerting (Monitoring); trace intents from Bayan Gateway to n8n and back. | Platform/SRE | Open |
