...
- We use **one shared database with row-level tenant isolation** (a `company_id` on every tenant table) rather than a database-per-tenant, because shared hosting cannot provision databases on demand and most buyers get exactly one database.
- The schema must be created and upgraded **entirely from the browser installer** (`/install`) — there is no `php artisan migrate` for the buyer — so the migration system has to be self-contained, idempotent at the table level, and report progress to a live console.
- Tenant isolation must be enforced **at the data layer and fail closed**, so the column conventions (a `company_id` FK on every tenant table) are not optional decoration; they are the substrate the `Model` tenant scope relies on.
- The business model is **data-driven** (unlimited plans, roles, permissions, AI providers without code changes), so several tables intentionally carry JSON columns and per-company catalogues.

Without a single written contract for these conventions, two engineers would pick different FK behaviors, forget the `company_id`, or break the installer. This document is that contract.
...
| `Model` | `app/Core/Model.php` | Active-record base. Adds the tenant scope (`$tenantScoped`, `$tenantColumn = 'company_id'`), `$fillable`, `$hidden`, `$casts`, and timestamps. |
...
3. **Primary keys: `id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT`.** A SaaS selling to thousands of tenants will exceed 32-bit ranges in busy tables (applications, activity_log, notifications); `BIGINT UNSIGNED` is the default everywhere. Two intentional exceptions: pure pivot tables use a **composite PK** (e.g. `permission_role (role_id, permission_id)`), and `password_resets` uses `email` as its PK.
4. **Foreign keys are real and explicit, with a deliberate ON DELETE strategy:**
   - **CASCADE** for owned child rows whose existence is meaningless without the parent — e.g. `memberships → companies`, `permission_role → roles`, `settings → companies`. Deleting the parent removes the children.
   - **SET NULL** for *optional* references where the row should survive the loss of the pointer — e.g. `memberships.invited_by → users`, `activity_log.user_id → users`, `roles.parent_id → roles`. The history/record remains; the link becomes NULL.
   - **RESTRICT** where deletion must be *blocked* to protect integrity — e.g. `companies.owner_id → users` (you cannot delete a user who still owns a company) and `subscriptions.plan_id → plans` (you cannot delete a plan that still has subscriptions). `ON UPDATE CASCADE` is set on all FKs.
5. **Per-company uniqueness.** Uniqueness on tenant tables is scoped to the tenant via composite unique keys, never a bare global unique: `roles (company_id, slug)`, `settings (company_id, key)`, `memberships (company_id, user_id)`, `ai_credentials (company_id, provider)`. Two different companies may each have a role slugged `recruiter`. Truly global tables keep simple unique keys (`users.email`, `companies.slug`, `plans.slug`, `permissions.key`).
6. **Timestamps.** Tenant and entity tables carry `created_at` / `updated_at TIMESTAMP NULL DEFAULT NULL`, written by the `Model` (`$timestamps = true`). Append-only logs (`activity_log`, `files`, `notifications`, `gateway_events`) carry only `created_at`. `password_resets.created_at` defaults to `CURRENT_TIMESTAMP` because it is written outside the Model.
7. **JSON columns for extensibility.** Where the spec promises "extensible without code changes," we use native `JSON`: `companies.settings`, `plans.features`, `plans.limits`, `ai_credentials.meta`, `onboarding_progress.completed_steps`, `activity_log.properties`, and the planned `*.data` / `*.criteria` / `*.line_items` columns. JSON is for sparse, schema-flexible data — never for fields we filter or join on (those get real columns).
8. **Enums for closed sets.** Lifecycle and status fields use `ENUM(...)` (e.g. `subscriptions.status`, `users.status`, `companies.status`) so invalid states are rejected by the database itself.
...
- **Tables**: lowercase `snake_case`, **plural** for entities (`users`, `companies`, `subscriptions`), **singular-joined** for pivots in `a_b` alphabetical-ish form (`permission_role`, `membership_role`, `user_role`). Log tables may be singular by convention (`activity_log`).
- **Columns**: `snake_case`. Foreign keys are `<singular_referenced>_id` (`company_id`, `user_id`, `plan_id`, `parent_id`, `invited_by` is the one semantic exception).
- **Constraints/indexes** follow Laravel-style auto-names so they are predictable: unique `<table>_<col>_unique`, index `<table>_<col>_index`, foreign `<table>_<col>_foreign`, composite uniques use a descriptive middle (`memberships_company_user_unique`, `roles_company_slug_unique`).
...
1. **One database, shared by all tenants.** There is no per-tenant database or schema. Isolation is by `company_id`.
2. **Every tenant-scoped table MUST have `company_id BIGINT UNSIGNED`** with an FK to `companies` and an index. A tenant table without `company_id` is a bug.
3. **`companies` cannot be tenant-scoped** — it *is* the tenant root; scoping it to itself is nonsensical. Its model sets `$tenantScoped = false`.
4. **GLOBAL tables** (`users`, `companies`, `permissions`, `plans`, `password_resets`, `roles` when `company_id IS NULL`, `migrations`) hold platform-wide data and are never auto-filtered by tenant.
...
7. **Money is `DECIMAL(10,2)` + currency**; lifecycle is an `ENUM`; secrets are encrypted before insert (never stored plaintext).
...
This section catalogues **all 36 tables**, classified and described. "Scope" is **GLOBAL** (platform-wide) or **TENANT** (carries `company_id`). Full column lists, types, and index/FK detail live in [06-ERD.md](06-ERD.md); this is the architectural inventory.
...
| 2 | `companies` | GLOBAL (tenant root) | The tenant itself. | `owner_id → users` (RESTRICT). |
| 3 | `memberships` | TENANT | Links a user to a company; the per-company "seat". | `company_id → companies` (CASCADE), `user_id → users` (CASCADE), `invited_by → users` (SET NULL). UQ `(company_id, user_id)`. |
| 4 | `roles` | GLOBAL when `company_id IS NULL`, else TENANT | RBAC roles with single-parent inheritance. | `company_id → companies` (CASCADE), `parent_id → roles` (SET NULL). UQ `(company_id, slug)`. |
| 5 | `permissions` | GLOBAL | The permission catalogue, referenced by `key`. | Linked via `permission_role`. |
| 6 | `permission_role` | GLOBAL | Which permissions a role grants. | PK `(role_id, permission_id)`, both CASCADE. |
| 7 | `membership_role` | TENANT (via membership) | Tenant roles held on a membership. | PK `(membership_id, role_id)`, both CASCADE. |
| 8 | `user_role` | GLOBAL | Global roles granted directly to a user (e.g. super-admin). | PK `(user_id, role_id)`, both CASCADE. |
| 9 | `plans` | GLOBAL | Data-driven subscription plans; features/limits as JSON. | Referenced by `subscriptions`. |
| 10 | `subscriptions` | TENANT | A company's subscription lifecycle. | `company_id → companies` (CASCADE), `plan_id → plans` (RESTRICT). |
| 11 | `ai_credentials` | TENANT | Per-tenant encrypted AI provider keys. | `company_id → companies` (CASCADE). UQ `(company_id, provider)`. |
| 12 | `password_resets` | GLOBAL | Hashed, expiring reset tokens (PK `email`). | None (keyed by email). |
| 13 | `settings` | TENANT | Per-company key/value settings. | `company_id → companies` (CASCADE). UQ `(company_id, key)`. |
| 14 | `onboarding_progress` | TENANT (nullable company) | Per-user onboarding flow progress. | `user_id → users` (CASCADE), `company_id → companies` (CASCADE, nullable). UQ `(user_id, company_id, flow)`. |
| 15 | `activity_log` | TENANT (nullable company) | Audit trail of security/business events. | `company_id → companies` (CASCADE, nullable), `user_id → users` (SET NULL). |
...
| # | Table | Scope | Purpose |
|---|-------|-------|---------|
| 17 | `jobs` | TENANT | Job postings (title, status draft→open→closed, openings, salary range). UQ `(company_id, slug)`. |
| 18 | `pipeline_stages` | TENANT | Hiring pipeline stages, per job or company-default template (`job_id` nullable). |
| 19 | `applications` | TENANT | A candidate (user) applying to a job; current stage and status. UQ `(company_id, job_id, user_id)`. |
...
| 28 | `notifications` | TENANT (nullable company) | Per-user in-app/email notifications. |
| 29 | `notification_preferences` | TENANT (nullable company) | Per-user channel preferences per type. UQ `(user_id, company_id, type, channel)`. |
...
| 34 | `api_tokens` | GLOBAL (nullable company) | Hashed REST API tokens with abilities. UQ `token_hash`. |
...
- **GLOBAL** (no `company_id`, never tenant-filtered): `users`, `companies`, `permissions`, `permission_role`, `user_role`, `plans`, `password_resets`, `migrations`, `gateway_events`, `queued_jobs`, `failed_jobs`, and `roles`/`api_tokens`/`notifications`/`notification_preferences`/`onboarding_progress` when their `company_id` is NULL.
- **TENANT** (carry `company_id`, auto-filtered by the Model scope): `memberships`, `membership_role` (via membership), `subscriptions`, `ai_credentials`, `settings`, `activity_log` (when company set), plus all recruitment tables (`jobs`–`evaluations`), `files`, `invoices`, `payments`, `payment_methods`.
...
- **Reading/writing tenant rows** is always subject to RBAC at the controller/service layer: `company.view`/`company.update`, `members.*`, `roles.view`/`roles.manage`, `billing.view`/`billing.manage`, `ai.view`/`ai.manage`, `settings.view`/`settings.manage` for built tables; the planned `jobs.*`, `applications.*`, `interviews.*`, `evaluations.*`, `files.*`, `notifications.view`, and `platform.*` groups for planned tables (see [07-RBAC.md](07-RBAC.md) and 11-Permissions-Matrix).
...
- **Application layer**: every write goes through the `Validator` with rule strings (`required`, `email`, `min`, `max`, `unique:table,column`, `exists:table,column`, `in:...`, `regex`). For example a new company validates `slug` `unique:companies,slug` and a member invite validates `email` `exists`/`unique` as appropriate.
...
3. **Deleting a company.** CASCADE removes its `memberships`, tenant `roles`, `settings`, `subscriptions`, `ai_credentials`, and (when added) its recruitment data — by design. The company's `users` survive (they are global) but lose that membership.
4. **Deleting a user who owns a company.** RESTRICT blocks it; the owner must be transferred first. This prevents orphaned tenants.
...
6. **NULL `company_id` rows** (`activity_log`, `onboarding_progress`, `notifications`) represent platform-level records; tenant-scoped queries must not assume `company_id` is always set on these.
...
- **Encryption at rest for secrets.** `ai_credentials.credentials` stores an AES-256-GCM (authenticated) ciphertext via `encrypt_value()`/`decrypt_value()`; plaintext keys never touch the database. The planned `payment_methods` stores only gateway **tokens**, never PANs.
...
- **Tenant isolation is a security control.** The `company_id` columns and the fail-closed Model scope prevent cross-tenant data leakage; FK + index on `company_id` are mandatory.
...
- **Audit.** `activity_log` records security-relevant changes (role grants, AI key updates, ownership changes) with actor, subject, IP, and a JSON property bag (see [38-Audit-System.md]).

## Performance

The indexing strategy is built around the dominant access pattern: **filter by `company_id`, then by status/foreign key.**

- **Every foreign key is indexed.** InnoDB requires it for FK enforcement and it accelerates joins (`subscriptions_company_id_index`, `memberships_user_id_index`, `roles_parent_id_index`, etc.).
- **Status/filter columns are indexed** because nearly every list view filters on them: `users_status_index`, `companies_status_index`, `memberships_status_index`, `subscriptions_status_index`, `permissions_group_index`, `plans_is_active_index`, `activity_log_action_index`.
- **Composite keys serve both uniqueness and lookup**: `(company_id, slug)`, `(company_id, key)`, `(company_id, user_id)`, `(company_id, provider)` are leftmost-prefixed on `company_id`, so the same index that enforces tenant-local uniqueness also supports `WHERE company_id = ?` scans.
- **Planned hot tables** get composite indexes matching their queries: `jobs (company_id, status)`, `applications (company_id, job_id, status, current_stage_id)`, `interviews (company_id, application_id, status)`, `notifications (user_id, read_at)`, `payments (company_id, gateway_reference)`.
- **FULLTEXT indexes** on the planned `jobs`/`applications` text columns power search behind the `SearchInterface` (see [28-Search-System.md](28-Search-System.md)), always combined with a `company_id` filter.
- **Pagination everywhere.** `QueryBuilder::paginate()` uses `LIMIT/OFFSET` so list endpoints never load whole tables; large logs (`activity_log`, `notifications`) are always paginated and time-ordered.
- **N+1 avoidance.** Models expose batch helpers and joins (e.g. `User::companies()` joins `memberships`) rather than per-row queries.
...
- **Convention tests**: assert every tenant model (`$tenantScoped = true`) maps to a table that actually has a `company_id` column with an FK + index; assert no MyISAM tables and that charset is `utf8mb4` everywhere.
- **FK behavior tests**: deleting a `company` cascades to its `memberships`/`settings`/`subscriptions`/`ai_credentials`; deleting an `invited_by` user nulls `memberships.invited_by`; deleting a user who owns a company is **rejected**; deleting a plan with subscriptions is **rejected**.
- **Uniqueness/race tests**: concurrent inserts that violate `(company_id, user_id)` / `(company_id, slug)` raise an integrity error and are handled gracefully.
...
- **Tenant sharding path.** Row-level isolation can evolve to "tenant → shard" mapping: the `company_id` already partitions every tenant table, so moving large tenants to dedicated databases is a routing change, not a re-modeling.
