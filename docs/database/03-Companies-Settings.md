# 03 — Companies & Settings (Domain D2)

The tenant root of HalaOps. **`companies` is the tenant** — every tenant-scoped
row in the entire platform carries a `company_id` that resolves here (DB-2). This
domain owns the company record itself plus the dedicated tables that the early
...
beside companies: platform `global_settings`, per-user `user_settings`, and
...
`company_id` FK, nothing is a hard-coded ENUM (status/type/category are
...
> **BUILT vs BLUEPRINT.** `companies` and `settings` exist today (migrations
> 0002, 0013, 0016). The blueprint **extends** `companies` (replacing its
> `status` ENUM with a config-driven `company_status_id` FK → `company_statuses`,
> and moving the inline `logo`/`settings` JSON into dedicated tables) and
> **generalizes** the built `settings` table into `company_settings`. Those are
...
- [02-RBAC-Membership](02-RBAC-Membership.md) (D1) — `roles` (invited role) and
  `memberships` (an accepted `company_invitations` row becomes a membership)
...
- [05-Subscriptions-Billing](05-Subscriptions-Billing.md) (D4) — `plans`,
  `subscriptions`, `invoices`, `invoice_items`, `currencies`; `company_billing`
...
- [09-AI-Notifications](09-AI-Notifications.md) (D8) — `ai_providers`,
  `ai_models`, `tenant_ai_keys` (BUILT as `ai_credentials`); `company_ai_settings`
...
- [11-Files-Queue-Analytics-Logs](11-Files-Queue-Analytics-Logs.md) (D10) —
  ...
  `company_storage`), and `activity_logs` (the audit trail for this domain)
...
erDiagram
    ...
    companies ||--|| company_ai_settings : "has 1"
    companies ||--|| company_storage : "has 1"
    companies ||--o{ company_integrations : "installs"
    companies ||--o{ company_domains : "publishes"
    companies ||--o{ company_invitations : "sends"

    currencies ||--o{ company_billing : "billing currency"
    countries ||--o{ company_billing : "legal country"
    countries ||--o{ companies : "primary country"
    languages ||--o{ companies : "default language"
    timezones ||--o{ companies : "default timezone"

    integrations ||--o{ company_integrations : "catalog entry"
    storage_providers ||--o{ company_storage : "provider"
    files ||--o{ company_branding : "logo/favicon asset"

    ai_providers ||--o{ company_ai_settings : "default provider"
    ai_models ||--o{ company_ai_settings : "default model"
    tenant_ai_keys ||--o{ company_ai_settings : "default key"

    roles ||--o{ company_invitations : "invited role"
    users ||--o{ company_invitations : "invited by"
    company_invitations ||--o| memberships : "accepted becomes"

    users ||--o{ user_settings : "personal prefs"
    companies ||--o{ mail_settings : "tenant mail (company_id NULL = platform)"

    companies {
        bigint id PK
        char uuid UK
        bigint owner_id FK
        bigint company_status_id FK
        string name
        string slug UK
    }
    company_statuses {
        bigint id PK
        char uuid UK
        bigint company_id FK "NULL = system default"
        string key
    }
    company_settings {
        bigint id PK
    ...
    user_settings {
        bigint id PK
        bigint user_id FK
        bigint company_id FK "NULL = global pref"
        string key
    }
    mail_settings {
        bigint id PK
        bigint company_id FK "NULL = platform default"
        string mailer
    }
```
...
### 9.1 `companies` — BUILT (extended by blueprint)

- **Purpose:** the tenant root; one row per customer organization. Every
  tenant-scoped row platform-wide carries a `company_id` resolving here.
- **Tenant-scoped?** It *is* the tenant (no `company_id` on itself; `owner_id`
...
| `company_status_id` | BIGINT UNSIGNED | NO | — | **BLUEPRINT** → `company_statuses.id` (RESTRICT); replaces the BUILT `status` ENUM('trial','active','suspended','canceled') |
...
| `logo` | VARCHAR(255) | YES | NULL | BUILT inline path; superseded by `company_branding.logo_file_id` |
| `settings` | JSON | YES | NULL | BUILT inline blob; superseded by `company_settings` rows |
...
- **Indexes:**
  - `companies_uuid_unique` → (`uuid`) — unique
  - `companies_slug_unique` → (`slug`) — unique
  - `companies_owner_id_index` → (`owner_id`) — FK index
  - `companies_company_status_id_index` → (`company_status_id`) — FK index (BLUEPRINT; replaces `companies_status_index`)
  - `companies_country_id_index` → (`country_id`) — FK index (BLUEPRINT)
  - `companies_language_id_index` → (`language_id`) — FK index (BLUEPRINT)
  - `companies_timezone_id_index` → (`timezone_id`) — FK index (BLUEPRINT)
  - `companies_deleted_at_index` → (`deleted_at`) — index (added in 0016)
  - `companies_name_fulltext` → (`name`) — FULLTEXT (BLUEPRINT; tenant search in admin/global search)
- **Foreign keys:**
  - `owner_id` → `users(id)` ON DELETE RESTRICT ON UPDATE CASCADE (a company must keep an owner; ownership transfer is an app operation)
  - `company_status_id` → `company_statuses(id)` ON DELETE RESTRICT ON UPDATE CASCADE *(BLUEPRINT)*
...
- **Relationships + cardinality:**
  - `users` 1—* `companies` (a user owns many companies via `owner_id`; each company has exactly one owner)
  - `company_statuses` 1—* `companies`
  - `companies` 1—* `company_settings`
  - `companies` 1—1 `company_branding` / `company_billing` / `company_ai_settings` / `company_storage`
  - `companies` 1—* `company_integrations` / `company_domains` / `company_invitations`
  - `companies` 1—* (almost every tenant table platform-wide via `company_id`)
- **Notes:** Config-driven status is the headline blueprint change — the BUILT
  `status` ENUM is removed in favor of `company_status_id` → `company_statuses`
  (system defaults `trial`/`active`/`suspended`/`canceled` seeded with
  `company_id` NULL; tenants/super-admin may add custom states). The wide inline
  ...
  relationship is a single FK; *members* (many users per company with roles) live
  in D1 `memberships`, not here.

### 9.2 `company_statuses` — BLUEPRINT

- **Purpose:** config-driven lifecycle states for a company (the workflow that
  replaces the BUILT `companies.status` ENUM). System defaults plus tenant/admin
  custom states.
...
| `company_id` | BIGINT UNSIGNED | YES | NULL | NULL = system default; else owning tenant → `companies.id` |
...
| `is_default` | TINYINT(1) | NO | 0 | default for new companies |
...
- **Indexes:**
  - `company_statuses_uuid_unique` → (`uuid`) — unique
  - `company_statuses_company_key_unique` → (`company_id`,`key`) — unique composite
  - `company_statuses_company_id_index` → (`company_id`) — FK index
- **Foreign keys:** `company_id` → `companies(id)` ON DELETE CASCADE ON UPDATE CASCADE (custom statuses removed with the tenant; NULL system rows unaffected).
- **Relationships + cardinality:** `company_statuses` 1—* `companies`.
- **Notes:** Standard per-entity status shape from the Bible §2.1. Status
  *changes* on a company are recorded in the polymorphic D0 `status_histories`
  (`subject_type='company'`).

### 9.3 `company_settings` — BUILT as `settings` (generalized by blueprint)
...
- **Tenant-scoped?** Yes (`company_id`). **Soft-delete?** No (config rows;
...
- **Keys:** PK(`id`); UNIQUE(`uuid`); UNIQUE(`company_id`,`key`) (the BUILT business key).
- **Indexes:**
  - `company_settings_uuid_unique` → (`uuid`) — unique
  - `company_settings_company_key_unique` → (`company_id`,`key`) — unique composite (BUILT `settings_company_key_unique`)
  - `company_settings_company_group_index` → (`company_id`,`group`) — composite (BLUEPRINT; load a group at once)
- **Foreign keys:** `company_id` → `companies(id)` ON DELETE CASCADE ON UPDATE CASCADE (BUILT `settings_company_id_foreign`).
- **Relationships + cardinality:** `companies` 1—* `company_settings`.
- **Notes:** Free-form key/value by design — anything *structured/relational*
  ...
  typed table in this domain rather than buried here; `company_settings` holds the
  long tail of simple toggles. `type` is a cast hint string (config-driven, not an
  ENUM per DB-4).

### 9.4 `company_branding` — BLUEPRINT
...
- **Tenant-scoped?** Yes (`company_id`, one row per company). **Soft-delete?** No.

| Column | Type | Null | Default | Notes |
|---|---|---|---|---|
| `id` | BIGINT UNSIGNED | NO | AUTO_INCREMENT | PK |
| `uuid` | CHAR(36) | NO | — | UNIQUE |
| `company_id` | BIGINT UNSIGNED | NO | — | → `companies.id`; UNIQUE (1:1) |
| `logo_file_id` | BIGINT UNSIGNED | YES | NULL | → `files.id` (D10, SET NULL) — replaces inline `companies.logo` |
...
- **Keys:** PK(`id`); UNIQUE(`uuid`); UNIQUE(`company_id`) (enforces 1:1).
- **Indexes:**
  - `company_branding_uuid_unique` → (`uuid`) — unique
  - `company_branding_company_id_unique` → (`company_id`) — unique (1:1)
  - `company_branding_logo_file_id_index` → (`logo_file_id`) — FK index
  - `company_branding_logo_dark_file_id_index` → (`logo_dark_file_id`) — FK index
  - `company_branding_favicon_file_id_index` → (`favicon_file_id`) — FK index
- **Foreign keys:**
...
- **Relationships + cardinality:** `companies` 1—1 `company_branding`; each branding row references up to three `files` (D10).
...
| `registration_number` | VARCHAR(60) | YES | NULL | company registration no. |
...
- **Keys:** PK(`id`); UNIQUE(`uuid`); UNIQUE(`company_id`) (1:1).
- **Indexes:**
  - `company_billing_uuid_unique` → (`uuid`) — unique
  - `company_billing_company_id_unique` → (`company_id`) — unique (1:1)
  - `company_billing_country_id_index` → (`country_id`) — FK index
  - `company_billing_currency_id_index` → (`currency_id`) — FK index
- **Foreign keys:**
  - `company_id` → `companies(id)` ON DELETE CASCADE ON UPDATE CASCADE
...
- **Relationships + cardinality:** `companies` 1—1 `company_billing`; `currencies` 1—* `company_billing`; `countries` 1—* `company_billing`.
- **Notes:** Separated from `companies` to keep the legal/tax block (3NF) out of
...
### 9.6 `company_ai_settings` — BLUEPRINT
...
- **Keys:** PK(`id`); UNIQUE(`uuid`); UNIQUE(`company_id`) (1:1).
- **Indexes:**
  - `company_ai_settings_uuid_unique` → (`uuid`) — unique
  - `company_ai_settings_company_id_unique` → (`company_id`) — unique (1:1)
  - `company_ai_settings_default_provider_id_index` → (`default_provider_id`) — FK index
  - `company_ai_settings_default_model_id_index` → (`default_model_id`) — FK index
  - `company_ai_settings_default_key_id_index` → (`default_key_id`) — FK index
  - `company_ai_settings_cost_currency_id_index` → (`cost_currency_id`) — FK index
- **Foreign keys:**
  - `company_id` → `companies(id)` ON DELETE CASCADE ON UPDATE CASCADE
...
- **Relationships + cardinality:** `companies` 1—1 `company_ai_settings`; references one provider/model/key (D8).
...
### 9.7 `company_storage` — BLUEPRINT

- **Purpose:** per-tenant storage configuration — which storage provider the
  company's files use, per-tenant credentials/bucket overrides, and the storage
...
- **Keys:** PK(`id`); UNIQUE(`uuid`); UNIQUE(`company_id`) (1:1).
- **Indexes:**
  - `company_storage_uuid_unique` → (`uuid`) — unique
  - `company_storage_company_id_unique` → (`company_id`) — unique (1:1)
  - `company_storage_storage_provider_id_index` → (`storage_provider_id`) — FK index
- **Foreign keys:**
  - `company_id` → `companies(id)` ON DELETE CASCADE ON UPDATE CASCADE
  - `storage_provider_id` → `storage_providers(id)` ON DELETE RESTRICT ON UPDATE CASCADE
- **Relationships + cardinality:** `companies` 1—1 `company_storage`; `storage_providers` 1—* `company_storage`.
...
### 9.8 `company_integrations` — BLUEPRINT

- **Purpose:** marketplace/integration *installs* per tenant — which catalog
  integration is enabled for the company, its config, encrypted OAuth/API
  credentials, and connection status.
- **Tenant-scoped?** Yes (`company_id`). **Soft-delete?** Yes (`deleted_at` — an
...
- **Keys:** PK(`id`); UNIQUE(`uuid`); UNIQUE(`company_id`,`integration_id`) (one install per integration per tenant).
- **Indexes:**
  - `company_integrations_uuid_unique` → (`uuid`) — unique
  - `company_integrations_company_integration_unique` → (`company_id`,`integration_id`) — unique composite
  - `company_integrations_integration_id_index` → (`integration_id`) — FK index
  - `company_integrations_integration_status_id_index` → (`integration_status_id`) — FK index
  - `company_integrations_installed_by_index` → (`installed_by`) — FK index
  - `company_integrations_company_status_index` → (`company_id`,`integration_status_id`) — composite (hot path: active installs)
  - `company_integrations_deleted_at_index` → (`deleted_at`) — index
- **Foreign keys:**
  - `company_id` → `companies(id)` ON DELETE CASCADE ON UPDATE CASCADE
...
- **Relationships + cardinality:** `companies` 1—* `company_integrations`; `integrations` 1—* `company_integrations`; `integration_status` 1—* `company_integrations`.
- **Notes:** **Catalog `integrations`** (marketplace listing: name, slug, vendor,
  ...
  company-integration specific. If D0/D10 later claim a generic
...
- **Indexes:**
  - `company_domains_uuid_unique` → (`uuid`) — unique
  - `company_domains_hostname_unique` → (`hostname`) — unique (global; a host maps to one tenant)
  - `company_domains_company_id_index` → (`company_id`) — FK index
  - `company_domains_domain_type_id_index` → (`domain_type_id`) — FK index
  - `company_domains_domain_status_id_index` → (`domain_status_id`) — FK index
  - `company_domains_company_primary_index` → (`company_id`,`is_primary`) — composite (resolve a tenant's primary domain)
  - `company_domains_deleted_at_index` → (`deleted_at`) — index
- **Foreign keys:**
  - `company_id` → `companies(id)` ON DELETE CASCADE ON UPDATE CASCADE
...
- **Relationships + cardinality:** `companies` 1—* `company_domains`; each hostname belongs to exactly one company.
- **Notes:** Pairs with `company_branding` to render white-label career sites.
...
### 9.10 `company_invitations` — BLUEPRINT

- **Purpose:** pending invitations to join a company — token, invited email, the
...
- **Keys:** PK(`id`); UNIQUE(`uuid`); UNIQUE(`token`); UNIQUE(`company_id`,`email`) (one open invite per email per tenant — re-invites reuse/replace the row).
- **Indexes:**
  - `company_invitations_uuid_unique` → (`uuid`) — unique
  - `company_invitations_token_unique` → (`token`) — unique
  - `company_invitations_company_email_unique` → (`company_id`,`email`) — unique composite
  - `company_invitations_role_id_index` → (`role_id`) — FK index
  - `company_invitations_invited_by_index` → (`invited_by`) — FK index
  - `company_invitations_user_id_index` → (`user_id`) — FK index
  - `company_invitations_invitation_status_id_index` → (`invitation_status_id`) — FK index
  - `company_invitations_membership_id_index` → (`membership_id`) — FK index
  - `company_invitations_company_status_index` → (`company_id`,`invitation_status_id`) — composite (list pending invites)
  - `company_invitations_expires_at_index` → (`expires_at`) — index (sweep expired)
  - `company_invitations_deleted_at_index` → (`deleted_at`) — index
- **Foreign keys:**
  - `company_id` → `companies(id)` ON DELETE CASCADE ON UPDATE CASCADE
...
- **Relationships + cardinality:** `companies` 1—* `company_invitations`; `roles` 1—* `company_invitations`; an accepted invitation 1—0..1 `memberships` (D1).
...
- **Notes:** The platform-level twin of `company_settings`. No `company_id` by
...
- **Purpose:** per-user personal preferences — UI density, notification opt-ins,
  locale/timezone overrides, dashboard layout — optionally scoped to a company
  context.
- **Tenant-scoped?** Optional — `company_id` NULL = a user-global preference,
  non-null = preference within that company context. Always user-scoped.
  **Soft-delete?** No.
...
| `company_id` | BIGINT UNSIGNED | YES | NULL | NULL = global pref; else → `companies.id` |
...
- **Keys:** PK(`id`); UNIQUE(`uuid`); UNIQUE(`user_id`,`company_id`,`key`) (one value per key per user per context).
- **Indexes:**
  - `user_settings_uuid_unique` → (`uuid`) — unique
  - `user_settings_user_company_key_unique` → (`user_id`,`company_id`,`key`) — unique composite
  - `user_settings_company_id_index` → (`company_id`) — FK index
  - `user_settings_user_group_index` → (`user_id`,`group`) — composite (load a group)
- **Foreign keys:**
  - `user_id` → `users(id)` ON DELETE CASCADE ON UPDATE CASCADE
  - `company_id` → `companies(id)` ON DELETE CASCADE ON UPDATE CASCADE
- **Relationships + cardinality:** `users` 1—* `user_settings`; optionally `companies` 1—* `user_settings`.
- **Notes:** The NULL-`company_id` convention mirrors `onboarding_progress` (a
  user is global, but the same user behaves differently per company). Notification
...
- **Purpose:** outbound mail configuration — SMTP/API mailer, host/credentials,
  from-name/address, reply-to. Platform default when `company_id` is NULL; a
  tenant override (custom sending domain / white-label email) when set.
- **Tenant-scoped?** Optional — `company_id` NULL = platform default, non-null =
...
| `id` | BIGINT UNSIGNED | NO | AUTO_INCREMENT | PK |
| `uuid` | CHAR(36) | NO | — | UNIQUE |
| `company_id` | BIGINT UNSIGNED | YES | NULL | NULL = platform default; else → `companies.id` |
...
- **Keys:** PK(`id`); UNIQUE(`uuid`); UNIQUE(`company_id`) (one mail config per tenant; one NULL-`company_id` platform row).
- **Indexes:**
  - `mail_settings_uuid_unique` → (`uuid`) — unique
  - `mail_settings_company_id_unique` → (`company_id`) — unique (tenant 1:1 / single platform row)
- **Foreign keys:** `company_id` → `companies(id)` ON DELETE CASCADE ON UPDATE CASCADE.
- **Relationships + cardinality:** `companies` 1—0..1 `mail_settings`; one global NULL-`company_id` row.
- **Notes:** All secrets (`password`, `api_key`) are encrypted at the app layer
  like `tenant_ai_keys`/`company_storage`. `is_verified` underpins white-label
  sending domains used by D8 notification delivery; the NULL-`company_id` row is
  the platform fallback. UNIQUE(`company_id`) makes both the platform default and
...
- **Company lifecycle** → `company_statuses` (per-entity status table; replaces
  the BUILT `companies.status` ENUM). System defaults seeded with `company_id`
...
- **Integration connection state** → `integration_status`; **domain
  ...
  `company_id` NULL for system rows, `key/label/color/sort_order/is_default/
...
Status *changes* on companies/invitations/integrations/domains are recorded in
