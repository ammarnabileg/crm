...
> **Status: Active (incremental build) | Version: 2.3.0 | Last updated: 2026-07-01 | Owner: Architecture (Nizam Core)**
...
| 3 | AI Runtime Engine + Plugin Architecture + Master Orchestrator | **In progress** — Plugin Platform and the AI Runtime + Master Orchestrator delivered & test-green |
...
**Status (2026-07-01).** **Plugin architecture** and the **AI Runtime Execution Engine + Master Orchestrator** are **delivered & test-green**. The `Nizam\Platform\Plugin\*` substrate (ADR-0016/0020) and the `Nizam\Runtime\*` execution core (ADR-0017/0018/0021) are implemented: a sole-entry `MasterOrchestrator`; a 13-state, event-sourced, state-machine-guarded, replayable `Execution` aggregate; worker isolation through the `WorkerCoordinator` (Sequential / in-process Parallel fan-out / Collaborative re-entry); `ResultMerger` + `ConfidenceEvaluator` → `ManagerDecision`; idempotency by `ExecutionId`, retry/timeout/recovery/replay; and InMemory + PDO (sqlite-validated) adapters with a tenant-scoped, soft-deleting repository and an append-only event store. All exit-criteria test paths (unit, replay, recovery, concurrency, end-to-end) are green. Remaining Phase-3 debt: Redis/true-distributed lock, multi-node parallel dispatch, Postgres CI validation, and the Execution Monitor UI. See [docs/26-AI-Runtime.md](./26-AI-Runtime.md) and [docs/audit/PHASE_RUNTIME_AUDIT.md](./audit/PHASE_RUNTIME_AUDIT.md).
...
| 2.2.0 | 2026-07-01 | Architecture (Nizam Core) | Recorded the **Plugin Platform** (`Nizam\Platform\Plugin\*`) as an implemented, test-green increment of Phase 3 (Runtime + Plugins + Orchestrator): the "everything is a plugin" substrate (ADR-0016) with its concrete mechanics (ADR-0020) — SDK contracts + nine kinds, self-validating manifest, strict SemVer 2.0.0 + constraints, reflection-checked kind↔entry-point validation, topological dependency resolution, permission-gated fault-catching in-process sandbox isolation, guarded lifecycle + events, health checks, InMemory + PDO (sqlite-validated) adapters, directory/array sources, container instantiator, and the tenant-aware `PluginManager` façade. Suite green at **347 tests / 967 assertions** (178 Plugin tests added) on PHP 8.4.19 + PHPUnit 11.5.55. OS-process isolation and the Interface/HTTP marketplace screen deferred. See docs/25-Plugin-Platform.md and docs/audit/PHASE_PLUGIN_AUDIT.md. |
| 2.2.1 | 2026-07-01 | Architecture (Nizam Core) | Re-verified the **Plugin Platform** increment (Phase 3) on a real run and refreshed the implementation audit (`docs/audit/PHASE_PLUGIN_AUDIT.md`). HONEST numbers confirmed: 70 source files `php -l` clean, `composer dump-autoload -o` clean, full suite green under `failOnWarning`/`failOnRisky` at **347 tests / 967 assertions** (Plugin: 178 = 160 unit + 18 integration) on PHP 8.4.19 + PHPUnit 11.5.55. No other phase advanced; verification-only. |
| 2.3.0 | 2026-07-01 | Architecture (Nizam Core) | Recorded the **AI Runtime Engine + Master Orchestrator** (`Nizam\Runtime\*`) as an implemented, test-green increment of Phase 3, realizing ADR-0017 (single Master Orchestrator), ADR-0018 (13-state execution state machine), and ADR-0021 (event-sourced persistence + advisory-row lock + idempotent execution): a sole-entry orchestrator; a state-machine-guarded, event-sourced, replayable `Execution` aggregate; worker isolation through the `WorkerCoordinator` (Sequential / in-process Parallel fan-out / Collaborative re-entry — never worker-to-worker); merge + confidence scoring → `ManagerDecision`; idempotency by `ExecutionId`, retry/timeout/recovery/replay; InMemory + PDO (sqlite-validated) adapters with a tenant-scoped, soft-deleting repository and an append-only event store. Suite green under `failOnWarning`/`failOnRisky` at **488 tests / 1378 assertions** on PHP 8.4.19 + PHPUnit 11.5.55 (Runtime: 141 = 128 unit + 13 integration; 140 source files, 28 folder READMEs, all `php -l` clean). Redis/true-distributed lock, multi-node parallel dispatch, Postgres CI validation, and the Execution Monitor UI deferred. See docs/26-AI-Runtime.md and docs/audit/PHASE_RUNTIME_AUDIT.md. |
