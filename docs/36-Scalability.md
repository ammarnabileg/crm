...
- **Secrets across nodes** — `APP_KEY` (which decrypts tenant `tenant_ai_keys`) and DB credentials are distributed via the environment/secret manager, never baked into images or logs; all nodes share one key so encrypted data is portable, and key rotation is coordinated fleet-wide.
